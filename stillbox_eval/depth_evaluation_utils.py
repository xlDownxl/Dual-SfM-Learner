...
class test_framework_stillbox(object):
    def __init__(self, root, test_files, seq_length=3, min_depth=1e-3, max_depth=80, step=1, use_gps=False):
        self.root = root
    ...
    def __getitem__(self, i):
        ...
        tgt = self.img_files[i][self.tgt_indices[i]]
        self.img_files[i].pop(self.tgt_indices[i])
        return {'ref': [imread(img).astype(np.float32) for img in self.img_files[i]],
                'tgt': imread(tgt).astype(np.float32),
...
def get_poses(scene, indices):
    ...
    try:
        if len(scene['orientation']) == 0:
            ...
            scene_quaternions = np.float32(scene['orientation'])[indices]
    except: 
        scene_quaternions = np.float32([1,0,0,0]).reshape(1,4).repeat(sl, axis=0)
