...
def main():
    ...
    disp_net = DispResNet(args.resnet_layers, False, num_input_images=2).to(device)
    ...
    for i in tqdm(range(1,len(test_files))):
        ...
        h, w, _ = ref_img.shape
        if (not args.no_resize) and (h != args.img_height or w != args.img_width):
            ref_img = resize(ref_img, (args.img_width,args.img_height,)).astype(np.float32)
        ref_img = np.transpose(ref_img, (2, 0, 1))

        tensor_img_ref = torch.from_numpy(ref_img).unsqueeze(0)
        tensor_img_ref = ((tensor_img_ref/255 - 0.45)/0.225).to(device)

        output = disp_net(torch.cat((tensor_img,tensor_img_ref),1))[0][0][0].cpu().numpy()
        depth=1/output
